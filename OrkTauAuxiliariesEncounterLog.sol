...
/// @title Ork Tau Auxiliaries Encounter Log
/// @notice Records short Ork descriptions of encounters with Tau Auxiliaries.
/// @dev Fully safe: no payments, no owner, no external calls, no selfdestruct.

contract OrkTauAuxiliariesEncounterLog {

    address constant EXAMPLE_ADDRESS = 0x0000000000000000000000000000000000000001;

    struct Encounter {
        string auxiliary;  // Kroot, Vespid, Gue'vesa, Demiurg, etc.
    ...
    event EncounterRecorded(uint256 indexed id, string auxiliary, address indexed creator);
    event EncounterVoted(uint256 indexed id, bool approved, uint256 approvedVotes, uint256 rejectedVotes);

    constructor() {
        logs.push(
            Encounter({
                auxiliary: "Example Auxiliary (Fill your own below)",
                behavior: "Describe how da auxiliary acted in da scrap.",
    ...
    function recordEncounter(
        string calldata auxiliary,
        string calldata behavior,
        string calldata outcome
    ) external {
        require(bytes(auxiliary).length > 0, "Auxiliary required");
        ...
        emit EncounterRecorded(logs.length - 1, auxiliary, msg.sender);
    }
